...
export const MetalDrawer = memo(function MetalDrawer({
    ...
    const showInnerBox = canMove || !(quality === 'low' && !isOpen)

    let handleSegments = 16
    if (quality === 'medium') handleSegments = 12
    else if (quality === 'low') handleSegments = 8
    ...
    const parts = useMemo<PartSpec[]>(() => {
        ...
        p.push({
            id: 'drawerFront',
            geometry: <boxGeometry args={[trayW, trayH, frontThickness]} />,
            position: [0, trayH / 2, zPanel] as Vec3,
            color,
            roughness: .45,
            metalness: .25,
        ...
        if (showInnerBox) {
            ...
            ;[-trayW / 2 + boxThickness / 2, +trayW / 2 - boxThickness / 2].forEach((x) =>
                p.push({
                    id: 'drawerBox',
        ...
        ;[-postX, postX].forEach((x) =>
            p.push({
                id: 'handle',
                geometry: <cylinderGeometry args={[hr, hr, hd, handleSegments]} />,
                position: [x, trayH * 0.55, out * (frontThickness + hoff - hd / 2)] as Vec3,
        ...
        p.push({
            id: 'handle',
            geometry: <cylinderGeometry args={[hr, hr, hw - 2 * hr, handleSegments]} />,
            position: [0, trayH * 0.55, out * (frontThickness + hoff)] as Vec3,
